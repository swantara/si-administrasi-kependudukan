        ...
        <div class="col-md-12">
          ...
          <div class="box box-danger">
            <div class="box-header with-border">
              ...
              <a href="<?=site_url('pindah')?>" class="btn btn-default"><i class="fa fa-arrow-left"></i> Kembali</a>
              ...
              <div class="row">
                ...
                <div class="col-md-3">
                  <div class="form-group">
                    ...
                    <input name="alamat_tujuan" id="alamat_tujuan" type="text" class="form-control input-sm" value="<?=set_value('alamat_tujuan')?>" placeholder="Alamat Tujuan">
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="form-group">
                    <small>Provinsi Tujuan</small>
  ...
  <script>
    ...
    function filter()
    {
      ...
      if($('#kedatangan_datepicker').val() != ""){
        tag.html(tag.html() + " Tanggal Pindah : <u>" + $('#kedatangan_datepicker').val() + "</u>, ");
      }
      if($('#alamat_tujuan').val() != ""){
        tag.html(tag.html() + " Alamat Tujuan : <u>" + $('#alamat_tujuan').val() + "</u>, ");
      }
      if($('#provinsi').val() != ""){
        <?php
          if(is_object($provinsi) || is_array($provinsi)) :
            ...
            foreach ($provinsi as $row) :
              $filter = $filter.'if($("#provinsi").val() == "'.$row->id_provinsi.'"){
                tag.html(tag.html() + " Provinsi Tujuan : <u>'.$row->provinsi.'</u>, ");
              }
              else ';
            endforeach;
            $filter = substr($filter, 0, strlen($filter) - 5);
          ...
          if(is_object($kabupaten) || is_array($kabupaten)) :
            ...
            foreach ($kabupaten as $row) :
              $filter = $filter.'if($("#kabupaten").val() == "'.$row->id_kabupaten.'"){
                tag.html(tag.html() + " Kabupaten Tujuan : <u>'.$row->kabupaten.'</u>, ");
              }
              else ';
            endforeach;
            $filter = substr($filter, 0, strlen($filter) - 5);
          ...
          if(is_object($kecamatan) || is_array($kecamatan)) :
            ...
            foreach ($kecamatan as $row) :
              $filter = $filter.'if($("#kecamatan").val() == "'.$row->id_kecamatan.'"){
                tag.html(tag.html() + " Kecamatan Tujuan : <u>'.$row->kecamatan.'</u>, ");
              }
              else ';
            endforeach;
            $filter = substr($filter, 0, strlen($filter) - 5);
          ...
          if(is_object($desa) || is_array($desa)) :
            ...
            foreach ($desa as $row) :
              $filter = $filter.'if($("#desa").val() == "'.$row->id_desa.'"){
                tag.html(tag.html() + " Desa Tujuan : <u>'.$row->desa.'</u>, ");
              }
              else ';
            endforeach;
            $filter = substr($filter, 0, strlen($filter) - 5);
          ...
          if(is_object($banjar) || is_array($banjar)) :
            ...
            foreach ($banjar as $row) :
              $filter = $filter.'if($("#banjar").val() == "'.$row->id_banjar.'"){
                tag.html(tag.html() + " Banjar Tujuan : <u>'.$row->banjar.'</u>, ");
              }
              else ';
            endforeach;
            $filter = substr($filter, 0, strlen($filter) - 5);
      ...
      if($('#no_kk').val() == "" && $('#nik').val() == "" && $('#nama_penduduk').val() == "" && $('#periode').val() == "" && $('#status_kk').val() == "" && $('#tempat_lahir').val() == "" && $('#datepicker').val() == "" && jk == "2" && $('#alamat').val() == "" && $('#agama').val() == "" && $('#pendidikan').val() == "" && $('#pekerjaan').val() == "" && $('#telepon').val() == "" && $('#nama_ayah').val() == "" && $('#nama_ibu').val() == "" && $('#status_kependudukan').val() == "" && $('#status_kawin').val() == "" && $('#kewarganegaraan').val() == "" && $('#kedatangan_datepicker').val() == "" && $('#alamat_tujuan').val() == "" && $('#provinsi').val() == "" && $('#kabupaten').val() == "" && $('#kecamatan').val() == "" && $('#desa').val() == "" && $('#banjar').val() == ""){
        tag.html(tag.html() + " Menampilkan seluruh data penduduk pindah.");
      }
      // console.log($('#datepicker').val());
      $.post( "http://localhost/si-administrasi-kependudukan/index.php/pindah/pencarianservice/", 
      { 
        no_kk: $('#no_kk').val(),
        nik: $('#nik').val(),
        ...
        pindah_datepicker: $('#kedatangan_datepicker').val(),
  ...
  {
    $.getJSON( "<?=site_url('pindah/ajaxgetkabupatenbyprovinsi/')?>" + input.value)
      .done(function( data ) {
        if(data != false)
        {
          kabupaten.empty();
  ...
  {
    $.getJSON( "<?=site_url('pindah/ajaxgetkecamatanbykabupaten/')?>" + input.value)
      .done(function( data ) {
        if(data != false)
        {
          kecamatan.empty();
  ...
  {
    $.getJSON( "<?=site_url('pindah/ajaxgetdesabykecamatan/')?>" + input.value)
      .done(function( data ) {
        if(data != false)
        {
          desa.empty();
  ...
  {
    $.getJSON( "<?=site_url('pindah/ajaxgetbanjarbydesa/')?>" + input.value)
      .done(function( data ) {
        if(data != false)
        {
          banjar.empty();
